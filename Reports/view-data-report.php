            ...
            <div class="col-lg-12 mb-0">
                            ...
                            <div class="form-group row pb-3">
                                ...
                                <div class="col-lg-6">
                                    <select data-plugin-selectTwo class="form-control populate" name="FormID"
                                            id="FormID" onchange="getModuleName(document.getElementById('FormID').value)" required>
                                ...
                                <div class="col-lg-6">
                                    <select data-plugin-selectTwo class="form-control populate" name="DivisionCode"
                                        ...
                                        foreach ($rsDivQuery as $row) {
                                            echo '<option value="' . $row->DivisionCode . '"'. (isset($DivisionCode) && !empty($DivisionCode) && $DivisionCode == $row->DivisionCode ? ' selected' : '') .'>' . $row->DivisionName . '</option>';
                                        }
                                        ?>
                                    </select>
                                </div>
                                ...
                                <div class="col-lg-6">
                                    <select data-plugin-selectTwo class="form-control populate"
                                        ...
                                        if ($loggedUserName == 'admin') {
                                            ...
                                            $qryDistUser = "SELECT id, UserName, FullName FROM userinfo WHERE IsActive = 1 AND UserName LIKE '$dataCollectorNamePrefix%' AND CompanyID = ? $filterPSUAssignedUser ORDER BY UserName ASC";
                                ...
                                <div class="col-lg-6">
                                    <div class="input-daterange input-group">
                                        <input type="date" class="form-control" id="startDate"
                                               name="startDate" value="<?php echo isset($StartDate) ? $StartDate : ''; ?>" required>
                                        <span class="input-group-text border-start-0 border-end-0 rounded-0">to</span>
                                        <input type="date" class="form-control" id="endDate" name="endDate" value="<?php echo isset($EndDate) ? $EndDate : ''; ?>" required>
                                    </div>
                                </div>
                            </div>
                ...
                if ($_REQUEST['show'] === 'Show') {
                    try {
                        ...
                        $MasterDataQuery = "SELECT ColumnName, ColumnLabel FROM xformcolumnname WHERE FormId = ? and ColumnName IN 
                                        (SELECT X.ColumnName FROM xformcolumnname X	
                                        INNER JOIN (SELECT ColumnName FROM ModuleInfo WHERE FormId = ? AND ModuleName = ? AND QuestionType='straight') M ON X.ColumnName LIKE CONCAT(M.ColumnName, '%')) 
                                        ORDER BY id";
                        $MasterDataRS = $app->getDBConnection()->fetchAll($MasterDataQuery, $FormID, $FormID, $moduleName);
                    ...
                    <section class="card">
                        ...
                        <header class="card-header">
                            ...
                            <div class="title">
                                Module Name:<?php echo $moduleName ?> ->DataStatus:<?php echo $DataStatus ?> </h3>
                                <?php
                                if ($DivisionCode) {
                            ...
                            <div class="table-responsive table-container">
                                <table class="table table-responsive-lg table-bordered table-striped table-sm mb-0" id="viewDataReport">
                                    ...
                                    <tr>
                                        ...
                                        $CallProcQuery = "EXEC spReport_MasterDataReport_Pivot '$FormID','$DataStatus', '$moduleName', '$StartDateTime', '$EndDateTime','$SelectedUserID','$DivisionCode','$DistrictCode','$UpazilaCode','$UnionWardCode','$MauzaCode','$VillageCode'";
                                        $CallProcOutput = db_query($CallProcQuery, $cn);
                                        $TotalRow = db_num_rows($CallProcOutput);
                                        $i = 1;

                                        while ($row = db_fetch_array($CallProcOutput)) {
                                            $dataUserId = $row['FUserID'];
                                            $dataUserName = getValue('userinfo', 'UserName', "id=$dataUserId");
                                            $dataUserInfo = "$dataUserName ($dataUserId)";

                                            echo "<tr>";
                                            echo "<td>" . $i++ . "</td>";
                                            echo "<td>" . $row['FinalxID'] . "</td>";
                                            echo "<td>" . $row['FPSU'] . "</td>";
                                            echo "<td>" . $dataUserInfo . "</td>";
